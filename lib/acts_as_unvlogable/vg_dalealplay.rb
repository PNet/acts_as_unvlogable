...
class VgDalealplay
  ...
  def initialize(url=nil, options={})
    ...
    @page =  Nokogiri::HTML(open(url).read("UTF-8"))
    #open(url) {|f| Hpricot(f.read.encode("UTF-8")) }
  end

  def title
    @page.xpath("//title").text.split(" - www.dalealplay.com")[0].to_s
  end
  ...
  def embed_url
    @page.xpath("//link[@rel='video_src']").first.attributes["href"].sub("autoStart=true", "autoStart=false")
  end
